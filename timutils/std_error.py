...
class MeanStdError(object):
    ...
    def __init__(self, arr):
        ...
        ----------
        arr : array-like
        	array of numeric values to calculate standard error of
        	mean
    ...
    def calc(self, dim=0):
        ...
        ----------
        dim : int
        	the dimension of self.arr along which the mean,
        	standard error should be calculated
        adjust_n : boolean
        	if True, use the "effective sample size" of Wilks
        	(1995) to account for autocorrelation in the data.
...
def calc_std_err(arr, dim=0, n=None):
    ...
    arr : array-like
    	numpy array of numeric values dim (int): the dimension of arr
    	along which the effective sample size should be
    	calculated. Default is 0.
    n : array-like
    	value of n to be used for each element of arr along axis
    	dim. Default is the number of elements along axis dim of arr
    	(i.e. arr.shape[dim]).  That is, the default is to treat each
    	element of arr as an independent data point.  A different
    	value of n may be useful to, for example, account for
    	autocorrelation in arr (see function calc_autocorr).

    Returns
    -------
    array-like
    	A numpy array containing standard error of the mean along axis
    	dim of arr.

    References
    ----------
    Efron, B. & Tibshirani, R., 1991.  Statistical Data Analysis in
    the Computer Age.  Science, 253, 5018, p 390- 395.
    """
    if n is None:
        n = arr.shape[dim]
...
def calc_neff(arr, dim=0):
    ...
    dim : int
    	the dimension of arr along which the effective sample size
    ...
    An Introduction.  Academic Press, New York
    """
...
def calc_autocorr(arr, dim=0, lag=1):
    ...
    arr : array-like
    	numpy array of numeric values
    dim : int
    	the dimension of arr along which the autocorrelation should be
    	calculated.  Default is 0.
    lag : int
    	lag to use for autocorrelation calculation.
