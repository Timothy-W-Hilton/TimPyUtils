...
def setup_colormap_with_zeroval(vmin, vmax, nlevs=5,
    ...
    ARGS:
        ...
        vmax (float): value to map to the highest color level
        nlevs (integer): number of levels for the colormap (default is 5)
        cmap (:class:`matplotlib.colors.Colormap` instance): colormap to
            use for the N-level colormap
        extend (string): ({'both'} | 'min' | 'max' | 'neither') should
           the top or bottom of the colormap use an arrow to indicate
           "and larger" or "and smaller"
